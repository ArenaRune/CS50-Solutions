...
function Player:calculateJumps()
    ...
    if self.dy < 0 then
        if self.map:tileAt(self.x, self.y).id ~= TILE_EMPTY or
            ...
            if self.map:tileAt(self.x, self.y).id == FLAG_STAT then
                self.map:setTile(math.floor(self.x / self.map.tileWidth) + 1,
                    math.floor(self.y / self.map.tileHeight) + 1, FLAG_END)
                playCoin = true
                endofgame = 1
            else
                playHit = true
            end
            if self.map:tileAt(self.x + self.width - 1, self.y).id == FLAG_STAT then
                self.map:setTile(math.floor((self.x + self.width - 1) / self.map.tileWidth) + 1,
                    math.floor(self.y / self.map.tileHeight) + 1, FLAG_END)
                playCoin = true
                endofgame = 1
